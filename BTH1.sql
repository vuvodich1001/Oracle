...
--29: CHO BIET KHACH HANG CO SO DON DAT HANG NHIEU NHAT
SELECT C.ID, C.NAME FROM S_CUSTOMER C JOIN S_ORD O ON C.ID = O.CUSTOMER_ID 
...
--31: HIEN THI HO, TEN VA NGAY TUYEN DUNG CUA TAT CA NHAN VIEN CUNG PHONG VOI LAN(KHÔNG TH?Y TÊN LAN NÊN LÀM TÊN MAI NHÉ)
SELECT LAST_NAME, FIRST_NAME, START_DATE FROM S_EMP 
WHERE DEPT_ID IN (SELECT DEPT_ID FROM S_EMP WHERE FIRST_NAME = 'Mai') AND FIRST_NAME != 'Mai';

--32: HIEN THI MANV, HO, TEN VA MA TRUY CAP CUA TAT CA NHAN VIEN CO MUC LUONG TEN TRUNG BINH
SELECT ID, LAST_NAME, FIRST_NAME, USERID FROM S_EMP
WHERE SALARY > (SELECT AVG(SALARY) FROM S_EMP);

--33: HIEN THI MANV, HO, TEN CUA NHAN VIEN CO MUC LUONG TEN TRUNG BINH VA CO CHU KI TU L
SELECT ID, LAST_NAME, FIRST_NAME, USERID FROM S_EMP
WHERE SALARY > (SELECT AVG(SALARY) FROM S_EMP) AND FIRST_NAME like '%L%';

--34: HIEN THI NHUNG KHACH HANG CHUA BAO GIO DAT HANG
SELECT ID, NAME FROM S_CUSTOMER
MINUS
SELECT O.CUSTOMER_ID ID, C.NAME NAME FROM S_ORD O JOIN S_CUSTOMER C ON C.ID = O.CUSTOMER_ID;
