...
CREATE TABLE INSTRUCTOR(
    ...
    CREATEDBY VARCHAR2(30) NOT NULL,
    CREATEDDATE DATE NOT NULL,
    MODIIEDBY VARCHAR2(30) NOT NULL,
    MODIFIEDDATE DATE NOT NULL
);
--5
CREATE TABLE CLASS(
...
ON ENROLLMENT
DECLARE
    STID STUDENT.STUDENTID%TYPE;
    DEM INT;
    CURSOR EN_CURSOR IS SELECT STUDENTID FROM STUDENT;
BEGIN
    OPEN EN_CURSOR;
    LOOP
        FETCH EN_CURSOR INTO STID;
        SELECT COUNT(*) INTO DEM FROM ENROLLMENT WHERE STUDENTID = STID;
        IF(DEM > 4) THEN
...
DECLARE
    MASV ENROLLMENT.STUDENTID%TYPE := 215;
    SOTIEN NUMBER(9);
BEGIN
